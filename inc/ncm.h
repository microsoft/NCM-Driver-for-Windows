...
{
    UINT32    Signature;                          // dwSignature ("NCMH" (NTH16) or "ncmh" (NTH32))
};
...
typedef struct _CDC_CONN_SPEED_CHANGE
{
    ...
    USB_CDC_NOTIFICATION    NotificationHeader;
    UINT32                  USBitRate;              // upstream bit rate
    UINT32                  DSBITRate;              // downstream bit rate
...
typedef struct _USB_CDC_UNION_DESCRIPTOR
{
    ...
    UCHAR       bDescriptorType;                    // CS_INTERFACE (USB_CS_INTERFACE_TYPE)
    UCHAR       bDescriptorSubtype;                 // USB_CS_INTF_SUBTYPE_CDC_UNION
    UCHAR       bControlInterface;
    UCHAR       bSubordinateInterface[1];           // variable-length (but for this driver, allow only one)
...
typedef struct _USB_ECM_CS_NET_FUNCTIONAL_DESCRIPTOR
{
    ...
    UCHAR bDescriptorType;                          // CS_INTERFACE (USB_CS_INTERFACE_TYPE)
    UCHAR bDescriptorSubtype;                       // descriptor subtype
    UCHAR iMACAddress;                              // index of MAC address string descriptor
    UCHAR bmEthernetStatistics[4];                  // ethernet stats bitmap
    UINT16 wMaxSegmentSize;                         // maximum segment size
    UCHAR wNumberMCFilters[2];                      // multicast filters bitmap
    UCHAR bNumberPowerFilters;                      // wake-up pattern filter count
...
typedef struct _USB_NCM_CS_FUNCTIONAL_DESCRIPTOR
{
    ...
    UCHAR bDescriptorType;                          // CS_INTERFACE (USB_CS_INTERFACE_TYPE)
    UCHAR bDescriptorSubtype;                       // descriptor subtype
    UCHAR bcdNcmVersion[2];                         // bcdNcmVersion
    UCHAR bmNetworkCapabilities;                    // network capabilities bitmap
...
typedef struct _USB_CDC_CS_UNION_DESCRIPTOR
{
    ...
    UCHAR bDescriptorType;                          // CS_INTERFACE (USB_CS_INTERFACE_TYPE)
    UCHAR bDescriptorSubtype;                       // descriptor subtype
    UCHAR bMasterInterface;                         // master interface's i/f number
    UCHAR bSlaveInterfaces[1];                      // slave interface set's i/f numbers
