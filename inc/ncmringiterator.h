...
protected:
    ...
    NcmOsQueue const * const    m_OsQueue;
    NET_RING* const             m_Ring;
    UINT32                      m_Index;
    const UINT32                m_Last;

    inline
    NcmOsRingIterator(NcmOsQueue const * const osQueue, NET_RING* const ring, UINT32 index, const UINT32 last)
        : m_OsQueue(osQueue), m_Ring(ring), m_Index(index), m_Last(last) {}

public:

    inline UINT32   GetIndex() const    { return m_Index; }
    inline BOOLEAN  HasAny()  const     { return m_Index != m_Last; }
    inline UINT32   GetCount() const    { return (m_Last - m_Index) & m_Ring->ElementIndexMask; }    
    inline void     Advance()           { m_Index = NetRingIncrementIndex(m_Ring, m_Index); }
    inline void     AdvanceEnd()        { m_Index = m_Last; } 
};

class NcmFragmentIterator : public NcmOsRingIterator
{
public:

    inline
    NcmFragmentIterator(NcmOsQueue const * const osQueue, UINT32 index, const UINT32 last)
        : NcmOsRingIterator(osQueue, NetRingCollectionGetFragmentRing(osQueue->RingCollection), index, last) {}

    inline NET_FRAGMENT* GetFragment() const { return NetRingGetFragmentAtIndex(m_Ring, m_Index); }

    inline NET_FRAGMENT_VIRTUAL_ADDRESS* const GetVirtualAddress() const
    {
...
class NcmPacketIterator : public NcmOsRingIterator
{
public:

    inline
    NcmPacketIterator(NcmOsQueue const * const osQueue, UINT32 index, const UINT32 last)
        : NcmOsRingIterator(osQueue, NetRingCollectionGetPacketRing(osQueue->RingCollection), index, last) {}

    inline NET_PACKET*  GetPacket() const   { return NetRingGetPacketAtIndex(m_Ring, m_Index); }

    inline NcmFragmentIterator GetFragmentIterator() const
    {
        NET_PACKET const * packet = GetPacket();
        UINT32 const last = NetRingAdvanceIndex(NetRingCollectionGetFragmentRing(m_OsQueue->RingCollection),
...
inline
NcmPacketIterator
NcmGetAllPackets(_In_ NcmOsQueue const * const osQueue)
{
...
inline
NcmFragmentIterator
NcmGetAllFragments(_In_ NcmOsQueue const * const osQueue)
{
...
inline
void
NcmReturnAllPacketsAndFragments(_In_ NcmOsQueue const * const osQueue)
{
...
)
{
    ...
    for (NcmFragmentIterator fi = pi->GetFragmentIterator(); 
         fi.HasAny(); 
         fi.Advance())
    {
